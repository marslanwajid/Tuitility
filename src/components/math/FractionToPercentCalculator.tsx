...
export default function FractionToPercentCalculator() {
  ...
  const [activeField, setActiveField] = useState<'numerator' | 'denominator' | 'whole'>('numerator');
  ...
  useEffect(() => {
    // Correct active field when switching tabs
    if (activeTab === 'simple' && activeField === 'whole') {
  ...
  }, [simpleFraction, mixedNumber, activeTab]);

  const handleFieldChange = (field: 'numerator' | 'denominator' | 'whole', val: string) => {
    const cleanVal = val.replace(/[^0-9]/g, ''); // positive integers only
    
    // Denominator cannot start with 0
  ...
  const adjustFieldValue = (field: 'numerator' | 'denominator' | 'whole', amount: number) => {
    const currentStr = activeTab === 'simple' 
      ? (field === 'whole' ? '0' : simpleFraction[field as 'numerator' | 'denominator'])
      : mixedNumber[field];
    ...
    if (field === 'whole' && newVal < 0) newVal = 0;

    const formatted = newVal.toString();
    if (activeTab === 'simple') {
      if (field !== 'whole') {
        setSimpleFraction(prev => ({ ...prev, [field]: formatted }));
      }
    } else {
      setMixedNumber(prev => ({ ...prev, [field]: formatted }));
    }
    setError('');
  };
  ...
  const handleKeyboardInput = (char: string) => {
    const currentStr = activeTab === 'simple'
      ? (activeField === 'whole' ? '0' : simpleFraction[activeField as 'numerator' | 'denominator'])
      : mixedNumber[activeField];
    ...
    // Double check denominator zero check
    if (activeField === 'denominator' && newVal === '0') return;

    if (activeTab === 'simple') {
      if (activeField !== 'whole') {
        setSimpleFraction(prev => ({ ...prev, [activeField]: newVal }));
      }
    } else {
      setMixedNumber(prev => ({ ...prev, [activeField]: newVal }));
    }
    setError('');
  };
  ...
  const computeResults = (): CalculationResult => {
    ...
    if (activeTab === 'simple') {
      num = parseInt(simpleFraction.numerator);
      den = parseInt(simpleFraction.denominator);

      if (isNaN(num) || isNaN(den) || den === 0) {
        throw new Error('Please enter valid numbers. Denominator cannot be zero.');
      }

      inputDisplay = `${num}/${den}`;
      steps.push(`**Step 1: Start with the simple fraction**`);
      steps.push(`Fraction: $$\\frac{${num}}{${den}}$$`);

    } else {
      whole = parseInt(mixedNumber.whole) || 0;
      num = parseInt(mixedNumber.numerator);
      den = parseInt(mixedNumber.denominator);

      if (isNaN(num) || isNaN(den) || den === 0) {
        throw new Error('Please enter valid numbers. Denominator cannot be zero.');
      }

      inputDisplay = `${whole} ${num}/${den}`;
      steps.push(`**Step 1: Start with the mixed number**`);
      steps.push(`Mixed Number: $$${whole}\\frac{${num}}{${den}}$$`);
      
      const improperNum = (whole * den) + num;
      steps.push(`\n**Step 2: Convert mixed number to an improper fraction**`);
      steps.push(`Multiply the whole number by the denominator, then add the numerator:`);
      steps.push(`$$\\text{Improper Numerator} = (${whole} \\times ${den}) + ${num} = ${improperNum}$$`);
      steps.push(`Improper Fraction: $$\\frac{${improperNum}}{${den}}$$`);

      num = improperNum;
    }

    const stepOffset = activeTab === 'simple' ? 2 : 3;
    const decimal = num / den;
    steps.push(`\n**Step ${stepOffset}: Convert the fraction to a decimal**`);
    steps.push(`Divide the numerator by the denominator:`);
    steps.push(`$$${num} \\div ${den} = ${decimal.toFixed(6).replace(/\.?0+$/, '')}$$`);

    const percentage = decimal * 100;
    steps.push(`\n**Step ${stepOffset + 1}: Convert the decimal to a percentage**`);
    steps.push(`Multiply the decimal value by 100 and add the percent symbol (\\%):`);
    steps.push(`$$${decimal.toFixed(6).replace(/\.?0+$/, '')} \\times 100 = ${percentage.toFixed(4).replace(/\.?0+$/, '')}\\%$$`);
  ...
  const renderPercentageGauge = (percentageVal: number) => {
    // Cap display circle at 100%, show extra values textually
    const displayPercentage = Math.min(100, Math.max(0, percentageVal));
    const radius = 40;
    const circ = 2 * Math.PI * radius; // ~251.3
    const strokeDashoffset = circ - (circ * displayPercentage) / 100;

    return (
      <div className="flex flex-col items-center p-3.5 bg-slate-50 border border-slate-100 rounded-2xl w-full min-w-[130px] max-w-[200px] transition-all">
        <span className="text-[10px] uppercase font-extrabold tracking-wider text-slate-450 mb-2.5">Percent Dial</span>
        <svg width="68" height="68" viewBox="0 0 100 100" className="drop-shadow-sm transition-all duration-300 hover:scale-105">
          <circle
            cx="50"
            cy="50"
            r={radius}
          ...
          <text
            ...
            y="56"
            textAnchor="middle"
            className="font-mono font-black text-xs fill-slate-800"
          >
            {displayPercentage.toFixed(0)}%
          </text>
        </svg>
        {percentageVal > 100 && (
          <span className="text-[9px] text-slate-400 italic font-bold mt-1.5">Total: {percentageVal.toFixed(1).replace(/\.0$/, '')}%</span>
        )}
      </div>
    );
  };
      ...
      <form onSubmit={handleCalculateSubmit} className="space-y-6">
        <div className="flex flex-wrap items-center justify-center gap-6">
          {activeTab === 'simple' ? (
            ...
            /* Mixed Number Input Card */
            <div className="flex items-center space-x-3 bg-white border border-slate-200/80 p-5 rounded-2xl shadow-sm hover:border-slate-350 transition-all">
              {/* Whole number */}
              <div className="flex flex-col items-center w-24">
                <span className="text-[9px] uppercase font-bold text-slate-400 mb-1">Whole</span>
                <div 
                  onClick={() => setActiveField('whole')}
                  className={`flex items-center justify-between w-full p-1 rounded-lg ${activeField === 'whole' ? 'bg-slate-50 ring-1 ring-slate-200' : ''}`}
                >
                  <button
                    type="button"
                    onClick={(e) => { e.stopPropagation(); adjustFieldValue('whole', -1); }}
                    className="w-5 h-5 rounded bg-slate-100 border border-slate-200 text-slate-500 hover:bg-slate-200 transition-colors flex items-center justify-center text-[10px] font-bold"
                  >
                    <i className="fas fa-minus text-[8px]"></i>
                  </button>
                  <input
                    ...
                    onChange={(e) => handleFieldChange('whole', e.target.value)}
                    className="w-8 bg-transparent text-center font-bold text-base focus:outline-none text-slate-900 mx-1 border-b border-transparent font-mono"
              ...
              {/* Fraction card */}
              <div className="flex flex-col items-center border-l border-slate-200 pl-4 w-28 relative">
                {/* Numerator */}
                <div 
                  onClick={() => setActiveField('numerator')}
                  className={`flex items-center justify-between w-full p-1 rounded-lg ${activeField === 'numerator' ? 'bg-slate-50 ring-1 ring-slate-200' : ''}`}
                ...
                {/* Division line */}
                <div className="w-full border-t border-slate-300 my-2.5"></div>
          ...
          <div className="bg-[#1a1a1a] text-white rounded-3xl p-6 space-y-5 text-center shadow-lg relative overflow-hidden">
            ...
            <div className="space-y-4 relative z-10">
              <div className="space-y-1.5">
                <span className="text-[10px] text-slate-400 font-extrabold uppercase tracking-widest block">Input Value</span>
                <div className="text-xl font-extrabold text-white flex justify-center py-1">
                  {activeTab === 'simple' ? (
                    <InlineMath math={`\\frac{${simpleFraction.numerator}}{${simpleFraction.denominator}}`} />
                  ) : (
                    <InlineMath math={`${mixedNumber.whole}\\frac{${mixedNumber.numerator}}{${mixedNumber.denominator}}`} />
                  )}
                </div>
              </div>

              <div className="space-y-1.5 border-t border-slate-800/80 pt-4">
                <span className="text-[10px] text-slate-400 font-extrabold uppercase tracking-widest block">Percentage Form</span>
              ...
              <div className="space-y-1.5 border-t border-slate-800/80 pt-4">
                <span className="text-[10px] text-slate-400 font-extrabold uppercase tracking-widest block">Decimal Form</span>
