...
export default function AudioFormatConverter() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    if (blob) {
      ...
      setDone(true);
      addToast(`Converted to ${format.toUpperCase()}!`, 'success');
    } else {
      addToast('Conversion failed. The file may be unsupported.', 'error');
    }
    setBusy(false);
  ...
  const handleDownload = useCallback(() => {
    ...
    a.href = resultUrl;
    a.download = `${file.name.replace(/\.[^.]+$/, '')}_converted.${format}`;
  ...
  const estimatedSize = useMemo(() => {
    if (!meta || format !== 'mp3') return null;
    const bytes = quality * 1000 * meta.duration / 8;
    ...
    <div className="w-full max-w-none bg-white border border-slate-100 rounded-3xl p-6 md:p-8 shadow-sm space-y-6 text-slate-800 animate-fade-in text-left">
      <div className="bg-[#1a1a1a] text-white rounded-2xl p-4 flex items-start space-x-3 relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none select-none flex items-center justify-center overflow-hidden">
          <span className="text-white/[0.03] text-[80px] italic font-black tracking-tighter">FORMAT</span>
        </div>
        <i className="fas fa-exchange-alt text-white text-base mt-0.5 relative z-10"></i>
        <div className="space-y-1 relative z-10">
          <span className="text-[10px] font-black text-white uppercase tracking-wider block">Privacy & Security</span>
          <p className="text-[10px] text-slate-300 font-semibold leading-relaxed">
            All conversion happens locally in your browser. No files are ever uploaded.
          </p>
        </div>
      </div>
      ...
      <div className="bg-[#1a1a1a] text-white rounded-2xl relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none select-none flex items-center justify-center overflow-hidden">
          <span className="text-white/[0.03] text-[120px] italic font-black tracking-tighter">CONVERT</span>
        </div>
        ...
        <div className="relative z-10 p-5 md:p-6 space-y-5">
          {!file ? (
            ...
            >
              <input ref={inputRef} type="file" accept="audio/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) handleFile(f); e.target.value = ''; }} />
              <i className="fas fa-cloud-upload-alt text-2xl text-slate-400 mb-2 block"></i>
              <p className="text-sm font-semibold text-slate-300">Upload an audio file</p>
              <p className="text-[10px] text-slate-500 mt-1">Supports MP3, WAV, AAC, OGG, FLAC</p>
            </div>
          ) : (
            <div className="bg-white/5 rounded-xl p-4 border border-white/10 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <i className="fas fa-file-audio text-slate-400 text-lg"></i>
                  <div>
                    <p className="text-sm font-bold text-white">{file.name}</p>
                    <p className="text-[10px] text-slate-400">{formatBytes(file.size)}</p>
                  </div>
                </div>
                <button onClick={handleReset} className="text-red-400 hover:text-red-300 text-sm"><i className="fas fa-times"></i></button>
              </div>
              {meta && (
              ...
              {format === 'mp3' && estimatedSize && <p className="text-[10px] text-slate-400">Estimated output: ~{estimatedSize}</p>}
            </div>
          )}
          ...
          {busy && (
            <div className="space-y-2">
              ...
              <p className="text-[10px] text-slate-400 text-center">Converting... {progress}%</p>
            </div>
          )}
            ...
            <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4 text-center space-y-3">
              ...
              <div className="flex justify-center gap-3">
                ...
                <button onClick={handleReset} className="px-4 py-2 rounded-xl bg-white/10 text-white border border-white/20 text-xs font-bold hover:bg-white/20 transition-colors">
                  <i className="fas fa-redo mr-1"></i>Convert Another
                </button>
              </div>
            </div>
          )}
          ...
          {file && !busy && !done && (
            <div className="flex gap-3">
              ...
              <button onClick={handleConvert} className="px-6 py-2.5 rounded-xl bg-white text-[#1a1a1a] text-xs font-black hover:bg-slate-200 transition-colors shadow-lg">
                <i className="fas fa-exchange-alt mr-1"></i>Convert
              </button>
            </div>
          )}
          ...
          <div className="bg-white/5 rounded-xl p-4 border border-white/10 flex items-center gap-3">
            ...
            <p className="text-[11px] text-slate-300">
              Need to calculate bitrates or extract audio from video? Try the <a href="/utility-tools/audio-bitrate-converter" className="text-white font-bold underline hover:text-slate-200">Bitrate Calculator</a> or <a href="/utility-tools/video-to-audio-extractor" className="text-white font-bold underline hover:text-slate-200">Video to Audio Extractor</a>.
            </p>
          </div>
        </div>
      </div>
