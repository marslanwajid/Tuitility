...
const STEPS = [
  { num: 1, title: 'Find Video', desc: 'Open TikTok and find the video you want to save.', img: '/images/step-1.png', aspect: 'aspect-video' },
  { num: 2, title: 'Copy Link', desc: 'Tap Share and select Copy Link.', img: '/images/tiktok-step-2.png', aspect: 'aspect-video' },
  { num: 3, title: 'Paste URL', desc: 'Paste the TikTok link in the input and click Download.', img: '/images/step-4.png', aspect: 'aspect-[4/5]' },
  { num: 4, title: 'Save', desc: 'Preview and save the watermark-free video to your device.', img: '/images/step-3.png', aspect: 'aspect-[4/5]' },
];
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (data.success && data.videoUrl) {
        ...
        addToast(data.error || 'Could not fetch video. It may be private or unavailable.', 'error');
      }
    } catch {
    ...
    <div className="w-full max-w-none bg-white border border-slate-100 rounded-3xl p-6 md:p-8 shadow-sm space-y-6 text-slate-800 animate-fade-in text-left">
      <div className="bg-[#1a1a1a] text-white rounded-2xl p-4 flex items-start space-x-3 relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none select-none flex items-center justify-center overflow-hidden">
          <span className="text-white/[0.03] text-[80px] italic font-black tracking-tighter">TIKTOK</span>
        </div>
        <i className="fab fa-tiktok text-white text-base mt-0.5 relative z-10"></i>
        <div className="space-y-1 relative z-10">
          <span className="text-[10px] font-black text-white uppercase tracking-wider block">Privacy & Security</span>
          <p className="text-[10px] text-slate-300 font-semibold leading-relaxed">
            Your TikTok links are processed securely through our API. No data is stored.
          </p>
        </div>
      </div>
      ...
      <div className="bg-[#1a1a1a] text-white rounded-2xl relative overflow-hidden">
        ...
        <div className="relative z-10 p-5 md:p-6 space-y-5">
          <div className="space-y-1">
            ...
            <p className="text-sm font-bold text-white">Paste a TikTok video URL</p>
          </div>
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              <input
                ...
                type="text"
                value={url}
                onChange={(e) => { setUrl(e.target.value); setVideoUrl(null); }}
          ...
          <div className="bg-white/5 rounded-xl p-4 border border-white/10 flex items-center gap-3">
            ...
            <p className="text-[11px] text-slate-300">
              Need to download from other platforms? Try the <a href="/utility-tools/converter-tools/reels-downloader" className="text-white font-bold underline hover:text-slate-200">Instagram Reels Downloader</a> or check our <a href="/utility-tools/audio-bitrate-converter" className="text-white font-bold underline hover:text-slate-200">Audio Bitrate Converter</a>.
            </p>
          </div>
        </div>
      </div>
