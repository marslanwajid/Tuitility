...
const computeResult = (
  ...
  const weeklySurplus = rateNum * 1100;
  const dailySurplus = Math.round(weeklySurplus / 7);
  let targetCalories = Math.round(maintenanceCalories + dailySurplus);

  const totalToGain = targetW - w;
  const weeksToGoal = Math.ceil(totalToGain / rateNum);
  ...
  if (rateNum > 0.75) {
    recommendations.push('Consider a slower weight gain rate (0.25-0.5 kg/week) to minimize fat gain and maximize lean muscle.');
  }
  recommendations.push('Focus on a moderate calorie surplus with nutrient-dense whole foods rather than processed calories.');
  recommendations.push('Prioritize protein intake — aim for 1.6-2.2g per kg of body weight daily to support muscle growth.');
  recommendations.push('Include strength training 3-4 times per week with progressive overload to stimulate muscle hypertrophy.');
  recommendations.push('Time your nutrition around workouts — eat a protein-rich meal within 2 hours after training.');
  recommendations.push('Track your food intake consistently to ensure you are hitting your calorie and protein targets.');
  recommendations.push('Stay consistent — muscle gain is slower than fat loss. Expect 0.25-0.5 kg (0.5-1 lb) per week of lean gains.');
  recommendations.push('Get adequate sleep (7-9 hours per night) for optimal muscle recovery and hormonal balance.');
  recommendations.push('Avoid dirty bulking — excess fat gain makes future cutting harder. Keep the surplus moderate.');
  if (currentBmi < 18.5) {
    recommendations.push('If you are underweight, consider consulting a registered dietitian or healthcare provider for personalized guidance.');
  }
...
export default function WeightGainCalculator() {
  ...
  const [weightLbs, setWeightLbs] = useState<number>(121);
  ...
  const validateTarget = (val: number) => {
    ...
    if (val <= current) {
      setTargetError('Target weight must be greater than current weight');
      return false;
    }
    if (val <= 0) {
      ...
      <form onSubmit={handleCalculate} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
          ...
          <div className="bg-slate-50 border border-slate-200/60 p-5 rounded-2xl shadow-sm flex flex-col gap-3 transition-all hover:border-slate-300">
            <label className="text-xs font-extrabold uppercase tracking-wider text-slate-500 block">Weekly Gain Rate</label>
        ...
        <div className="pt-8 border-t border-slate-150 space-y-8 animate-fade-in-up">
          ...
          <div className="bg-[#1a1a1a] text-white rounded-3xl p-6 md:p-8 space-y-6 text-center shadow-lg relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-6">
              ...
              <div className="pt-2 pb-6 border-b border-slate-800/80">
                <span className="text-[10px] text-slate-400 font-extrabold uppercase tracking-widest block mb-1">Weight Gain Target</span>
                ...
                <span className="inline-block mt-2 px-3 py-1 rounded-full text-xs font-black tracking-wide uppercase border text-emerald-400 border-emerald-500/40 bg-emerald-500/10">
                  +{result.dailySurplus} kcal/day surplus
                </span>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3 pb-4 border-b border-slate-800/80">
                ...
                <div className="text-center">
                  <span className="text-xs text-slate-500 font-bold uppercase tracking-wider block">To Gain</span>
                  <span className="font-mono font-black text-base text-emerald-400">+{(result.targetWeight - result.currentWeight).toFixed(1)} <span className="text-xs font-medium text-slate-400">{result.weightUnit}</span></span>
                </div>
              </div>
              ...
              <table className="w-full min-w-[400px] text-xs border-collapse text-left">
                <thead className="sticky top-0 bg-white z-10">
                  <tr className="bg-slate-50 border-b border-slate-200 text-slate-400 font-bold uppercase tracking-wider text-xs">
                    ...
                    <th className="px-6 py-3.5">Gained This Week</th>
                    <th className="px-6 py-3.5">Total Gained</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 text-slate-700">
                  {result.projection.map((row, idx) => {
                    const weeklyGain = idx === 0 ? 0 : parseFloat((row.weight - (result.projection[idx - 1]?.weight || result.currentWeight)).toFixed(1));
                    const totalGained = parseFloat((row.weight - result.currentWeight).toFixed(1));
                    return (
                      <tr key={idx} className={`hover:bg-slate-50/55 transition-colors duration-250 ${idx === result.projection.length - 1 ? 'bg-slate-900/5 font-black text-slate-900' : ''}`}>
                        ...
                        <td className="px-6 py-3.5 font-mono text-slate-500">{idx === 0 ? '—' : `+${weeklyGain.toFixed(1)} ${result.weightUnit}`}</td>
                        <td className={`px-6 py-3.5 font-mono font-bold ${totalGained > 0 ? 'text-emerald-600' : ''}`}>{idx === 0 ? '—' : `+${totalGained.toFixed(1)} ${result.weightUnit}`}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <div className="bg-slate-50 border border-slate-150 rounded-3xl p-6 md:p-8 space-y-4 text-left">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium">Tips for a successful weight gain journey</p>
              </div>
            </div>
          ...
          <div className="space-y-4 text-left">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <span className="text-[10px] text-slate-400 font-mono">Mifflin-St Jeor + Surplus</span>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-slate-50 border border-slate-150 rounded-2xl p-5 space-y-2">
                <span className="text-xs font-extrabold uppercase tracking-wider text-slate-500">Surplus & Target</span>
                <div className="font-mono text-sm font-bold text-slate-800 leading-relaxed">
                ...
                <p className="text-[10px] text-slate-500 font-medium">Weekly gain (kg) × 1100 = weekly calorie surplus</p>
              </div>
            </div>
          </div>
