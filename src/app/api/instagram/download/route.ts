...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!reelMatch) {
      return NextResponse.json({ success: false, error: 'Invalid Instagram Reel URL format' }, { status: 400 });
    }
    ...
    if (videoUrl) {
      return NextResponse.json({ success: true, videoUrl, reelId: reelMatch[1] });
    }

    return NextResponse.json({
      success: false,
      error: 'Could not retrieve video. The reel may be private or the link is invalid.',
    }, { status: 404 });
