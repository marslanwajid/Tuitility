class LCMCalculatorLogic {
  ...
  validateCalculation(formData) {
    ...
    if (!numbersInput) {
      return { valid: false, error: 'Please enter numbers' };
    }
    ...
    if (numbers.length < 2) {
      return { valid: false, error: 'Please enter at least two valid positive numbers' };
    }

    if (numbers.some(n => n > 1000000)) {
      return { valid: false, error: 'Numbers must be less than 1,000,000 for performance reasons' };
    }
  ...
  calculateByListing(numbers) {
    let steps = [
      ...
      `Value: (${numbers.join(', ')})`,
      'Find and list multiples of each number until the first common multiple is found.'
    ];
    ...
    numbers.forEach(num => {
      let multipleString = `• Multiples of ${num}: ${num}`;
      for (let i = 2; i <= Math.min(5, Math.floor(lcmValue/num)); i++) {
        multipleString += `, ${num * i}`;
      }
      multipleString += `, ..., ${lcmValue}`;
      steps.push(multipleString);
    });
  ...
  calculateByCakeLadder(numbers) {
    ...
    while (!currentNumbers.every(n => n === 1)) {
      ...
      let rowStep = `Prime: ${smallestPrime} | Numbers: ${currentNumbers.join(', ')}`;
      tableSteps.push(rowStep);
      
      currentNumbers = currentNumbers.map(n => {
        return n % smallestPrime === 0 ? n / smallestPrime : n;
      });
    ...
    return { result, steps, tableData: { primeFactors, numbers } };
  }
  ...
  calculateByDivision(numbers) {
    let steps = [
      ...
      `Value: (${numbers.join(', ')})`,
      'Write the numbers in the top row.',
      'Divide the numbers by prime numbers until all become 1.'
    ];
    ...
    while (!currentNumbers.every(n => n === 1)) {
      ...
      let rowStep = `Divisor: ${divisor} | Numbers: ${currentNumbers.join(', ')}`;
      tableSteps.push(rowStep);
  ...
  calculate(formData) {
    ...
    try {
      ...
      return {
        ...
        tableData: calculationResult.tableData || null,
        error: null
      };
    } catch (error) {
      return {
  ...
  getCalculationMethodLabel(value) {
    const method = this.calculationMethods.find(m => m.value === value);
